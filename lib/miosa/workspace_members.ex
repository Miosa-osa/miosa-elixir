defmodule Miosa.WorkspaceMembers do
  @moduledoc """
  ...
  def list(%Client{} = client, workspace_id) when is_binary(workspace_id) do
  ...
  """
  @spec add(Client.t(), String.t(), String.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def add(%Client{} = client, workspace_id, user_id, role \\ "member") do
    body = %{"user_id" => user_id, "role" => role}

    case Client.post(client, "/workspaces/#{workspace_id}/members", body) do
      {:ok, %{"data" => data}} -> {:ok, data}
      {:ok, data} -> {:ok, data}
      {:error, _} = err -> err
    end
  ...
  """
  @spec update_role(Client.t(), String.t(), String.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def update_role(%Client{} = client, workspace_id, user_id, role) do
    body = %{"role" => role}

    case Client.patch(client, "/workspaces/#{workspace_id}/members/#{user_id}", body) do
      {:ok, %{"data" => data}} -> {:ok, data}
      {:ok, data} -> {:ok, data}
      {:error, _} = err -> err
    end
  ...
  """
  @spec remove(Client.t(), String.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def remove(%Client{} = client, workspace_id, user_id) do
    Client.delete(client, "/workspaces/#{workspace_id}/members/#{user_id}")
  end
end
